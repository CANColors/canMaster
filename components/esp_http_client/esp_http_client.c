...
static int http_on_message_begin(http_parser *parser)
{
    ...
    client->is_chunk_complete = false;
     http_dispatch_event(client, HTTP_EVENT_ON_MESSAGE_BEGIN, NULL,0);
    return 0;
}
...
static int http_on_body(http_parser *parser, const char *at, size_t length)
{
   ...
   /* char* buf = calloc(length+10, sizeof(char));
    memcpy(buf, client->response->buffer->raw_data, length);
    
     ESP_LOGD(TAG, "http_on_body raw_data===>\n %s", buf);
     free(buf);*/
...
char*  esp_http_client_get_content_data(esp_http_client_handle_t client, int* len)
{
   // *buf = client->response->buffer->output_ptr;
   //*buf = client->response->buffer->raw_data + client->response->data_offset;
   *len = client->response->data_process; 
  //  return     client->response->buffer->raw_len;
  return client->response->buffer->raw_data;
